...
import { seedAdjustmentTasks } from '@/ai/flows/seed-adjustment-tasks-flow';
...
export default function SettingsPage() {
  ...
  const [isSeedingAdjustment, setIsSeedingAdjustment] = useState(false);
  const { toast } = useToast();

  const anySeedingInProgress = isSeedingStudio || isSeedingSocietal || isSeedingAdjustment;
  ...
  const handleSeedAdjustmentTasks = async () => {
    setIsSeedingAdjustment(true);
    try {
      const result = await seedAdjustmentTasks();
      toast({
        title: "Blueprint Seeded",
        description: `${result.seeded} tasks have been added to the 'adjustmentTasks' collection in Firestore.`,
      });
    } catch (error) {
      console.error("Error seeding Adjustment tasks:", error);
      toast({
        variant: "destructive",
        title: "Seeding Failed",
        description: "There was an error seeding the Adjustment blueprint.",
      });
    } finally {
      setIsSeedingAdjustment(false);
    }
  };
        ...
        >
            <Card className="bg-card/50 backdrop-blur-sm">
                <CardHeader>
                    ...
                    <CardDescription>
                        Seed the project blueprints into Firestore to track completion. This is a one-time operation per blueprint.
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex items-center space-x-4 p-4 border rounded-lg bg-background">
                        <div className="flex-1">
                            <h3 className="font-semibold">Firebase Studio Blueprint</h3>
                            <p className="text-sm text-muted-foreground">
                                Injects the master checklist for the cockpit buildout.
                            </p>
                        </div>
                        <Button onClick={handleSeedStudioTasks} disabled={anySeedingInProgress}>
                     ...
                     <div className="flex items-center space-x-4 p-4 border rounded-lg bg-background">
                         <div className="flex-1">
                            <h3 className="font-semibold">Societal Singularity Blueprint</h3>
                            <p className="text-sm text-muted-foreground">
                            Injects the governance blueprint for societal evolution.
                            </p>
                        </div>
                        <Button onClick={handleSeedSocietalTasks} disabled={anySeedingInProgress} variant="secondary">
                    ...
                    <div className="flex items-center space-x-4 p-4 border rounded-lg bg-background">
                         <div className="flex-1">
                            <h3 className="font-semibold">Adjustment & Improvement Blueprint</h3>
                            <p className="text-sm text-muted-foreground">
                              Injects the refinement and hardening tasks for the systems.
                            </p>
                        </div>
                        <Button onClick={handleSeedAdjustmentTasks} disabled={anySeedingInProgress} variant="secondary">
                            {isSeedingAdjustment ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                            Seed Adjustment Tasks
                        </Button>
                    </div>
                </CardContent>
            </Card>
